...
public class IapSample : MonoBehaviour {
	...
	void Start () 
	{
		...
		if (currentActivity != null) {
			...
			// (2) Activity Context
			// (3) debug 여부
			iapRequestAdapter = new AndroidJavaObject("com.onestore.iap.unity.RequestAdapter", "Game_System", currentActivity, false); //Release
			//iapRequestAdapter = new AndroidJavaObject("com.onestore.iap.unity.RequestAdapter", "Game_System", currentActivity, true); //Debug
		}
	}
	...
	public void RequestPaymenet(string appcode, string iapcode, string iapname, string viewcode)
	{
		// ---------------------------------
		// 함수 parameter 정리
		// ---------------------------------
		...
		iapRequestAdapter.Call ("requestPayment", appcode, iapcode, iapname, viewcode, "BPINFO_0123");
		Debug.Log ("--------------------------------------------------------");
		Debug.Log ("[UNITY] RequestPaymenet >>> " + appcode);
		Debug.Log ("--------------------------------------------------------");
	}
	...
	public void PaymentResponse(string response) 
	{
		...
		if (data.result.code.Equals ("0000")) {
			GameObject.Find ("Game_System").GetComponent<Game_System> ().Coin += Pay;
			GameObject.Find ("Game_System").GetComponent<Game_System> ().SaveCoin();
		}
